...
describe('Likes API', () => {
    ...
    describe('GET /likes/movie/:movieId', () => {
        it('should return likes for a valid movie ID', async () => {
            ...
            const res = await request(app)
                .get(`/likes/movie/${movieId}`);
            console.log(res);
            expect(res.status).toBe(200);
            console.log('after status check');
            expect(res.body).toEqual({ message: 'Likes fetched successfully', data: movieLikes });
            console.log('after response check');
        });

        it('should return 400 for an invalid movie ID', async () => {
            const invalidMovieId = 'invalidId';

            const res = await request(app).get(`/likes/movie/${invalidMovieId}`);

            expect(res.status).toBe(400);
            expect(res.body).toEqual({ error: 'Invalid movieId' });
        });
    ...
    describe('POST /likes/toggleLikeReview', () => {
        it('should toggle like for a valid review ID', async () => {
            ...
            expect(res.status).toBe(200);
            expect(res.body).toEqual(expect.any(Object), 200, 'Review liked successfully');
        });
    ...
    describe('POST /likes/toggleLikeComment', () => {
        it('should toggle like for a valid comment ID', async () => {
            ...
            expect(res.status).toBe(200);
            expect(res.body).toEqual(expect.any(Object), 200, 'Comment liked successfully');
        });
    ...
    describe('POST /likes/toggleLikeMovie', () => {
        it('should toggle like for a valid movie ID', async () => {
            ...
            expect(res.status).toBe(200);
            expect(res.body).toEqual(expect.any(Object), 200, 'Movie liked successfully');
        });
    ...
    describe('POST /likes/toggleLikePost', () => {
        it('should toggle like for a valid post ID', async () => {
            ...
            expect(res.status).toBe(200);
            expect(res.body).toEqual(expect.any(Object), 200, 'Post liked successfully');
        });
