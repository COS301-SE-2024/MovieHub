...
fs.readFile('merged_output.json', 'utf8', (err, data) => {
  ...
  const containsSpanking = (movieData) => {
    ...
    return fieldsToCheck.some(field => movieData[field] && movieData[field].toLowerCase().includes('spanking'));
  };
  ...
  fs.writeFile('filtered_merged_output.json', filteredData.join('\n'), 'utf8', (err) => {
    if (err) {
